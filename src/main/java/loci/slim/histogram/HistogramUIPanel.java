 ...
 * LUT range. It also has checkboxes to control how the ranges are derived and
 * displayed.
 *
...
public class HistogramUIPanel extends JPanel {
	...
	public HistogramUIPanel(final boolean hasChannels) {
		...
		_minTextField.addActionListener(new ActionListener() {
		...
		_maxTextField.addActionListener(new ActionListener() {
		...
		_autoRangeCheckBox.addItemListener(new ItemListener() {
		...
		_excludePixelsCheckBox.addItemListener(new ItemListener() {
		...
		_combineChannelsCheckBox.addItemListener(new ItemListener() {
		...
		_displayChannelsCheckBox.addItemListener(new ItemListener() {
		...
		_logarithmicDisplayCheckBox.addItemListener(new ItemListener() {
		...
		final boolean experimental =
			userPreferences.getBoolean(EXPERIMENTAL, false);
		userPreferences.putBoolean(EXPERIMENTAL, experimental);
		if (experimental) {
			final JLabel experimentalLabel = new JLabel("Experimental:");
			add(experimentalLabel);

			_smoothingCheckBox = new JCheckBox(SMOOTHING, _smoothing);
			_smoothingCheckBox.addItemListener(new ItemListener() {
			...
			_family1RadioButton.addActionListener(new ActionListener() {
			...
			_family2RadioButton.addActionListener(new ActionListener() {
			...
			_bandwidthRadioButton.addActionListener(new ActionListener() {
			...
			_bandwidthTextField =
				new JTextField("" + HistogramPanel.DEFAULT_BANDWIDTH);
			_bandwidthTextField.addActionListener(new ActionListener() {
			...
			_bandwidthTextField.addFocusListener(new FocusAdapter() {
...
//        IJ.log("UIPanel.setMinMaxLUT " + min + " " + max);
	 ...
	 */
	private void enableUI(final boolean auto) {
	 ...
	 * shown.
	 */
	private void showMinMaxLUT(final double min, final double max) {
		String text;

		final DoubleFormatter minFormatter = new DoubleFormatter(true, DIGITS, min);
		text = minFormatter.getText();
		_minTextField.setText(text);
		_minLUT = parse(text);

		final DoubleFormatter maxFormatter =
			new DoubleFormatter(false, DIGITS, max);
		text = maxFormatter.getText();
		_maxTextField.setText(text);
		_maxLUT = parse(text);
	}

	private double parse(final String text) {
		double value = 0.0;
		// check for values of infinity
		final int index = text.indexOf(DoubleFormatter.INFINITY);
		if (0 == index) {
