...
public class HistogramTool {
	...
	private final static String LUT = "lifetime.lut"; // use a specific LUT; most
																										// IJ LUTs unsuitable
	...
	/**
	 * Gets an IndexColorModel by loading a hardcoded LUT file. This is just a
	 * temporary expedient, really belongs elsewhere.
	 *
	...
	public static IndexColorModel getIndexColorModel() {
		...
		try {
			...
			IJ.showMessage("Missing LUT",
				"Install lifetime.lut from LOCI FIJI update site.");
		...
		colorModel =
			PaletteFix.fixIndexColorModel(colorModel, Color.BLACK, Color.BLACK);
		return colorModel;
	}
...
//		IJ.log("----");
		...
		if (null != _frame) {
			...
			_colorBarPanel.setMinMax(minMaxView[0], minMaxView[1], minMaxLUT[0],
				minMaxLUT[1]);
		}
	...
	private int valueToPixel(final double value) {
		synchronized (_synchObject) {
			...
			final double max = minMaxView[1];
			final int pixel =
				(int) (PaletteFix.getSize() * (value - min) / (max - min));
			return pixel;
		}
	}
	 ...
	 */
	private void changed(final double minView, final double maxView,
	...
	private class HistogramDataListener implements IHistogramDataListener {
		...
		@Override
		public void minMaxChanged(final HistogramDataGroup histogramDataGroup,
			final double minView, final double maxView, final double minLUT,
			final double maxLUT)
		{
		...
		private class PeriodicTask extends TimerTask {
			...
			public void run() {
				...
				synchronized (_synchObject) {
					...
					_histogramPanel.setStatistics(_histogramDataGroup
						.getStatistics(WIDTH));
					_colorBarPanel.setMinMax(minView, maxView, minLUT, maxLUT);
	...
	private Dimension getSizeFromPreferences() {
		...
		return new Dimension(prefs.getInt(WIDTH_KEY, SET_WIDTH), prefs.getInt(
			HEIGHT_KEY, SET_HEIGHT));
	}
