...
public class RapidLifetimeDetermination {
	...
	private final Integrator integrator = new DefaultIntegrator();
	 ...
	 */
	int gciTripleIntegral(final double xincr, final double[] y,
		final int fit_start, final int fit_end, final int noise,
		final double[] sig, final double[] z, final double[] a, final double[] tau,
		final double[] fitted, final double[] residuals, final double[] chisq,
		final int division)
	{
		...
		a[0] =
			d12 /
				(tau[0] * exp_t0_tau * (1 - 2 * exp_dt_tau + exp_dt_tau * exp_dt_tau));
	...
	int gciTripleIntegralInstr(final double xincr, final double[] y,
		final int fit_start, final int fit_end, final double[] instr,
		final int ninstr, final int noise, final double[] sig, final double[] z,
		final double[] a, final double[] tau, final double[] fitted,
		final double[] residuals, final double[] chisq, final int division)
	{
		double d1, d2, d3, d12, d23;
		double t0, dt, exp_dt_tau, exp_t0_tau;
		int width;
	...
	int gciTripleIntegralFittingEngine(final double xincr, final double[] y,
		final int fit_start, final int fit_end, final double[] instr,
		final int ninstr, final int noise, final double[] sig, final double[] z,
		final double[] a, final double[] tau, final double[] fitted,
		final double[] residuals, final double[] chisq, final double chisq_target)
	{
		int tries = 1;
		int division = 3;
		final double[] local_chisq = new double[] { 3.0e38 };
		...
		if (0 == ninstr) {
			returnCode =
				gciTripleIntegral(xincr, y, fit_start, fit_end, noise, sig, z, a, tau,
					fitted, residuals, local_chisq, division);
			IJ.log("tries " + tries + " a t z chisq " + a[0] + " " + tau[0] + " " +
				z[0] + " " + local_chisq[0] + " returns " + returnCode);

			while (local_chisq[0] > chisq_target && local_chisq[0] <= old_chisq &&
				tries < MAX_REFITS)
			{
				...
				++tries;
				returnCode =
					gciTripleIntegral(xincr, y, fit_start, fit_end, noise, sig, z, a,
						tau, fitted, residuals, local_chisq, division);
				IJ.log("tries " + tries + " a t z chisq " + a[0] + " " + tau[0] + " " +
					z[0] + " " + local_chisq[0] + " returns " + returnCode);
			}
		}
			...
			returnCode =
				gciTripleIntegralInstr(xincr, y, fit_start, fit_end, instr, ninstr,
					noise, sig, z, a, tau, fitted, residuals, local_chisq, division);
			IJ.log("instr tries " + tries + " a t z chisq " + a[0] + " " + tau[0] +
				" " + z[0] + " " + local_chisq[0] + " returns " + returnCode);
	...
	public int rldFit(final ICurveFitter fitter, final ICurveFitData data) {
		...
		final double[] chisquare = new double[1];
		...
		final int stop = data.getAdjustedTransEndIndex();
		final double[] trans = data.getAdjustedTransient();
		...
		final int chiSquareAdjust = stop - start - 3;

		final double xinc = fitter.getXInc();
		int ninstr = 0;
		final double[] instrumentResponse = fitter.getInstrumentResponse(1);
		final double[] instrumentResponseFloat = null;
		if (null != instrumentResponse) {
			ninstr = instrumentResponse.length;
		}

		final double[] fitted = new double[trans.length];
		final double[] residuals = new double[trans.length];
		final int noise = 3;

		final int returnValue =
			gciTripleIntegralFittingEngine(xinc, trans, start, stop,
				instrumentResponseFloat, ninstr, noise, null, z, a, tau, fitted,
				residuals, chisquare, data.getChiSquareTarget() * chiSquareAdjust);
	...
	// Based on:
	// Error Analysis of the Rapid Lifetime Determination Method for
	// Double-Exponential Decays and New Windowing Schemes
	// Kristin K. Sharman and Ammasi Periasamy, et al
	// Analytic Chemistry, Vol. 71, No. 5, Mark 1, 1999

	private int testDoubleRLDFitOverlapping(final double offset,
		final double[] decay, final double xinc, final int start, final int stop)
	{
		final int width = stop - start; // start here is exclusive

		// split decay up into fifths
		final int width5Divisible = (width / 5) * 5;
		final int fifthWidth = width5Divisible / 5;

		IJ.log("width " + width + " width5Divisible " + width5Divisible +
			" fifthWidth " + fifthWidth);

		// sum photons in each fifth of the range
		final double[] fifth = new double[5];
		for (int i = 0; i < 5; ++i) {
			// get total count of photons in this interval
			final int startIndex = start + i * fifthWidth;
			fifth[i] =
				integrator.integrate(decay, offset, xinc, startIndex, startIndex +
					fifthWidth);
		}
		double sum = 0.0;
		for (int i = 0; i < 5; ++i) {
		...
		IJ.log("sum from start " + start + " to stop " + (start + width5Divisible) +
			" is " + sum);
		...
		final double r = d1 * d1 - d2 * d0;
		IJ.log("r " + r + " = d1 " + d1 + " * d1 " + d1 + " - d2 " + d2 + " * d0 " +
			d0);
		final double p = d1 * d2 + d3 * d0;
		IJ.log("p " + p + " = d1 " + d1 + " * d2 " + d2 + " + d3 " + d3 + " * d0 " +
			d0);
		final double q = d2 * d2 - d3 * d1;
		IJ.log("q " + q + " = d2 " + d2 + " * d2 " + d2 + " - d3 " + d3 + " * d1 " +
			d1);
		final double disc = p * p - 4.0 * r * q;
		IJ.log("disc " + disc + " = p " + p + " * p " + p + " - 4.0 * r " + r +
			" * q " + q);
		final double sqrtDisc = Math.sqrt(disc);
		IJ.log("sqrt disc " + sqrtDisc + " = Math.sqrt disc " + disc);
		final double x = (-p - sqrtDisc) / (2 * r);
		IJ.log("x " + x + " = (-p " + p + " - sqrtDisc " + sqrtDisc +
			") / (2 * r " + r + ")");
		final double y = (-p + sqrtDisc) / (2 * r);

		IJ.log("y " + y + " = (-p " + p + " + sqrtDisc " + sqrtDisc +
			") / (2 * r " + r + ")");

		final double deltaT = 2 * fifthWidth * xinc;
		IJ.log("deltaT " + deltaT + " = 2 * fifthWidth " + fifthWidth + " * xinc " +
			xinc);

		IJ.log("delta T is " + deltaT + " y squared is " + (y * y) +
			" x squared is " + (x * x));

		// compute taus (26)-(27)
		final double t1 = -deltaT / Math.log(y * y);
		final double t2 = -deltaT / Math.log(x * x);
		IJ.log("RLD overlapping T1 is " + t1 + " T2 is " + t2);
		return 0;
	}

	private int testDoubleRLDFit(final double offset, final double[] decay,
		final double xinc, final int start, final int stop)
	{
		final int width = stop - start; // start here is exclusive

		// split decay up into fourths
		final int width4Divisible = (width / 4) * 4;
		final int fourthWidth = width4Divisible / 4;

		IJ.log("width " + width + " width4Divisible " + width4Divisible +
			" fourthWidth " + fourthWidth);

		// sum photons in each fifth of the range
		final double[] fourth = new double[4];
		for (int i = 0; i < 4; ++i) {
			// get total count of photons in this interval
			final int startIndex = start + i * fourthWidth;
			fourth[i] =
				integrator.integrate(decay, offset, xinc, startIndex, startIndex +
					fourthWidth);
		}
		...
		IJ.log("sum from start " + start + " to " + (start + width4Divisible) +
			" is " + sum);
		...
		final double deltaT = 2 * fourthWidth * xinc;
		IJ.log("deltaT " + deltaT + " = 2 * fourthWidth " + fourthWidth +
			" * xinc " + xinc);

		// compute taus (22)-(23)
		final double t1 = -deltaT / Math.log(y);
		final double t2 = -deltaT / Math.log(x);
	...
	/**
	 * See
	 * "4.1 Resolution of a biexponential decay by Prony's method in the time domain"
		...
		final double xinc, final int start, final int stop)
	{
		...
		final double f0 = decay[0] - offset;
		final double f1 = decay[fourthWidth] - offset;
		final double f2 = decay[2 * fourthWidth] - offset;
		final double f3 = decay[3 * fourthWidth] - offset;
		...
		final double alpha1 = (f3 * f0 - f2 * f1) / (f1 * f1 - f2 * f0);
		final double alpha2 = (f3 * f1 - f2 * f2) / (f0 * f2 - f1 * f1); // TODO eq
																																			// 7
																																			// actually
																																			// has f0
																																			// * f2 *
																																			...
																																			// the
																																			// denominator
		// alpha2 = (f3 * f1 - f2 * f2) / (f0 * f2 * f2 - f1 * f1); //TODO, okay try
		// that, to no avail
		final double tau1 = -alpha1 / 2 + Math.sqrt((alpha1 * alpha1 / 4 - alpha2));
		final double tau2 = -alpha1 / 2 - Math.sqrt((alpha1 * alpha1 / 4 - alpha2));
	...
	public int trialRldFit(final ICurveFitter fitter, final ICurveFitData data) {
		final double chiSquareDelta = 0.01f;
		final double[] chisquare = new double[1];
		final double[] z = new double[1];
		final double[] a = new double[1];
		final double[] tau = new double[1];
		...
		final double offset = 0.53; // from SPC Image fitting Brian's _40a.sdt at
																// (142, 215)
