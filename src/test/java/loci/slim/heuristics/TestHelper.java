...
public class TestHelper {
	...
	{
		final double percent = percentageDifference(expResult, result);
		if (percent > margin) {
			fail("results differed (by " + percent + "& > " + margin +
				"%); expected:<" + expResult + "> but was:<" + result + ">");
		}
	}

	/**
	 * JUnit does provide a AssertEquals method to compare double arrays, i.e.:
	 * <p>
		...
		final double[] result, final double margin)
	{
		if (expResult.length != result.length) {
			fail("array lengths differed, expected.length=" + expResult.length +
				" actual.length=" + result.length);
		}
		for (int i = 0; i < result.length; ++i) {
			final double percent = percentageDifference(expResult[i], result[i]);
			if (percent > margin) {
				fail("arrays first differed (by " + percent + "% > " + margin +
					"%) at element [" + i + "]; expected:<" + expResult[i] +
					"> but was:<" + result[i] + ">");
			}
		}
	}

	private static double percentageDifference(final double expResult,
		final double result)
	{
